...
<input type="hidden" name="l_receiver" value="${dto.l_receiver}">
	...
	<tr>
			<td colspan="6" style="text-align: center;">
				<a href="sendletterlist.do?&pnum=<%=map.get("prePageNum")%>">◀</a>
				<%
					int pcount=(Integer)request.getAttribute("pcount");
					for(int i=map.get("startPage");i<=map.get("endPage");i++){
						%>
						<a href="sendletterlist.do?&pnum=<%=i%>" style="text-decoration: none;"><%=i%></a>				
						<%
						}					
				%>
				<a href="sendletterlist.do?&pnum=<%=map.get("nextPageNum")%>">▶</a>
			</td>
		</tr>	
	<tr>
	<tr>
